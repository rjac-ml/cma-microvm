...
Calls RunMicrovm through the boto3 ``lambda-microvms`` client. Upstream
``boto3``/``botocore`` (>= 1.43.40) ship the ``lambda-microvms`` service model,
so the client is available to the launcher at runtime with no vendored wheels —
the container image installs boto3 straight from PyPI.
"""
